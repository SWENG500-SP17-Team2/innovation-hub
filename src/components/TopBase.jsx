...
const TopBase = ({children}) => (
    <div>
        <AppBar title="Innovation Hub" iconElementLeft={< IndexLink to = "/" > <IconButton><NavigationHome/></IconButton> < /IndexLink>} iconElementRight= {LocalAuth.isUserAuthenticated() ? ( <div> <Link to="/NewPost"> <FlatButton label="Post New Idea"/> </Link> <Link to="/logout"> <FlatButton label="Sign out"/> </Link> <Link to="/ProfilePage"> <IconButton><AccountCircle /></IconButton> </Link> </div> ) : ( <div> <Link to="/login"> <FlatButton label="Login"/> </Link> <Link to="/signup"> <FlatButton label="Sign up"/> </Link> </div> )}/> {children}
